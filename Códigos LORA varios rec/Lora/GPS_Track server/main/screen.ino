...
void _screen_header() {
    ...
    // Message count
    snprintf(buffer, sizeof(buffer), "# SAT");
    ...
    if(axp2101_found && millis()%8000 < 3000){
        snprintf(buffer, sizeof(buffer), "%dV Bat:%d%%", axp_2101.getBattVoltage()/1000, axp_2101.getBatteryPercent());
...
void screen_show_logo() {
    ...
    uint8_t x = (display->getWidth() - S2P_IMAGE_WIDTH) / 2;
    uint8_t y = SCREEN_HEADER_HEIGHT + (display->getHeight() - SCREEN_HEADER_HEIGHT - S2P_IMAGE_HEIGHT) / 2 + 1;
    display->drawXbm(x, y, S2P_IMAGE_WIDTH, S2P_IMAGE_HEIGHT, S2P_IMAGE);
}
...
void screen_loop() {
    ...
    if (axp192_found && pmu_irq) {
        pmu_irq = false;
        axp_192.readIRQ();
        if (axp_192.isChargingIRQ()) {
            baChStatus = "Charging";
        } else {
            baChStatus = "No Charging";
        }
        if (axp_192.isVbusRemoveIRQ()) {
            baChStatus = "No Charging";
        }
    ...
    if (axp2101_found && pmu_irq) {
        pmu_irq = false;
        axp_2101.getIrqStatus();
        if (axp_2101.isBatChargeStartIrq()) {
            baChStatus = "Charging";
        } else {
            baChStatus = "No Charging";
        }
        if (axp_2101.isVbusRemoveIrq()) {
            baChStatus = "No Charging";
        }
        Serial.println(baChStatus); //Prints charging status to screen
        digitalWrite(2, !digitalRead(2));
